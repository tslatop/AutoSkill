...
```text
文档输入
  -> ingest（DocumentRecord + TextUnit + StrictWindow + 基于 hash 的增量跳过）
  -> extract（SupportRecord + SkillDraft）
  -> compile（SkillSpec）
  -> versioning + registry（lifecycle / provenance / history）
  -> staging 快照 + 最终 SkillBank store + 可见 family 技能树
```

- 旧的 `autoskill/offline/document` 已迁移到顶层 `AutoSkill4Doc/`。
- 文档 pipeline 现在是独立入口：使用 `autoskill4doc ...` 或 `python3 -m AutoSkill4Doc ...`。
- 当前 CLI 提供分阶段命令：`build`、`llm-extract`、`ingest`、`extract`、`compile`、`diag`、`retrieve-hierarchy`、`canonical-merge`、`migrate-layout`。
...
- `autoskill/`：SDK 核心实现。
- `AutoSkill4Doc/`：独立的离线文档到技能引擎，包含自身 CLI、配置、窗口切分、抽取、编译和版本管理模块。
- `examples/`：可直接运行的示例入口。
...
- 文档离线 pipeline 现在由 `AutoSkill4Doc/` 独立维护，不再通过 `autoskill.offline` 路由。
